...
open class MyServiceClass<Provider: DataServiceProvider>: DataService<Provider> {
    ...
    open func updateSalesOrderStatus(id: String, newStatus: String, query: DataQuery = DataQuery()) throws -> Bool? {
        return try BooleanValue.optional(self.executeQuery(query.invoke(MyServiceClassMetadata.ActionImports.updateSalesOrderStatus, ParameterList(capacity: (2 as Int)).with(name: "id", value: StringValue.of(id)).with(name: "newStatus", value: StringValue.of(newStatus)))).result)
    }
